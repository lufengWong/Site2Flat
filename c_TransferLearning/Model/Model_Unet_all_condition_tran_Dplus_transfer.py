...
class GeneratorUNet_ConTran_Frozen(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, pic_sizes=256, length_num=16):
        super(GeneratorUNet_ConTran_Frozen, self).__init__()
...
class TransferUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        u_output = self.final(u7)

        return u_output


class GeneratorUNet_ConTran(nn.Module):
    def __init__(self, in_ch, out_ch, pic_size, length_label):
        super(GeneratorUNet_ConTran, self).__init__()

        self.unet_1 = GeneratorUNet_ConTran_Frozen(in_channels=in_ch,
                                            out_channels=out_ch,
                                            pic_sizes=pic_size,
                                            length_num=length_label)

        self.unet_2 = GeneratorUNet_ConTran_Frozen(in_channels=in_ch,
                                            out_channels=out_ch,
                                            pic_sizes=pic_size,
                                            length_num=length_label)

        self.transfer = TransferUNet(in_channels=2 * in_ch,
                                     out_channels=out_ch, )
    ...
    def forward(self, x, c):
        ...
        g_1_plus_2 = torch.cat((g_1, g_2), dim=1)  # 在通道维度上进行拼接

        g_last = self.transfer(g_1_plus_2)
...
if __name__ == '__main__':
    ...
    transfer_unet = GeneratorUNet_ConTran(3, 3, 256, 3).to(device)

    #  #######
    Path_site = r'\site_params_generator.pth'
    state_dict_site = torch.load(Path_site)  # Path 为模型文件的保存路径

    Path_flat = r'\flat_params_generator.pth'
    state_dict_flat = torch.load(Path_flat)  # Path 为模型文件的保存路径

    # 新的名字的参数
    state_dict_site_new_unet1 = {}
    for key, value in state_dict_site.items():
        state_dict_site_new_unet1['unet_1.'+key] = value

    state_dict_flat_new_unet2 = {}
    for key, value in state_dict_flat.items():
        state_dict_flat_new_unet2['unet_2.' + key] = value

    # 修改参数
    model_dict = transfer_unet.state_dict()  # 获取自己定义的模型的参数
    model_dict.update(state_dict_site_new_unet1)  # 更新自己模型的参数
    model_dict.update(state_dict_flat_new_unet2)  # 更新自己模型的参数
    transfer_unet.load_state_dict(model_dict) # 参数参数

    model_dict_last = transfer_unet.state_dict()

    # 固定指定的参数
    list_frozen = list(state_dict_site_new_unet1.keys()) + list(state_dict_flat_new_unet2.keys())
    print(list_frozen)

    # 载入预训练模型参数后...
    for name, value in transfer_unet.named_parameters():
        if name in list_frozen:
            value.requires_grad = False

    # setup optimizer
    params = filter(lambda p: p.requires_grad, transfer_unet.parameters())
