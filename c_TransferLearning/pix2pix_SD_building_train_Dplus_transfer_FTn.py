...
from Model.Model_Unet_all_condition_tran_Dplus_transfer import *
...
def D_train(D: Discriminator, G: GeneratorUNet_ConTran, X, Label, BCELoss, optimizer_D):
    ...
    D_fake_label_loss = BCELoss(conSin_fake, torch.ones(conSin_fake.size()).to(device))
...
def D_val(D: Discriminator, G: GeneratorUNet_ConTran, X, Label, BCELoss, optimizer_D):
    ...
    conSin_fake = F.cosine_similarity(Label, D_output_label_f).to(device)
    conSin_fake = torch.mean(F.tanh(conSin_fake + 1.0).to(device))
    D_fake_label_loss = BCELoss(conSin_fake, torch.ones(conSin_fake.size()).to(device)).to(device)
...
def G_train(D: Discriminator, G: GeneratorUNet_ConTran, X, Label, BCELoss, L1, optimizer_G,
    ...
    # 反向传播并优化
    G_loss = G_BCE_loss + lamb * G_L1_Loss + lamb2 * conSin_real_G
    G_loss.backward()
    optimizer_G.step()
...
def main():
    # 过程数据储存
    stage = 'Building'
    project_name = '1007-Pix2Pix_Unet_SD-GAN-D_Plus_epoch500_building_2LearnD_transfer_FTn'
    ...
    # 数据来源
    root = r'\dataset_SD-GAN'  # r'F:\pix2pix\dataset\facades' # r'F:\pix2pix\dataset_building'
    ...
    # 修改参数
    model_dict = G.state_dict()  # 获取自己定义的模型的参数
    model_dict.update(state_dict_site_new_unet1)  # 更新自己模型的参数
    model_dict.update(state_dict_flat_new_unet2)  # 更新自己模型的参数
    G.load_state_dict(model_dict)  # 参数参数

    # 固定指定的参数
    # 微调层也不冻结
    state_dict_site_new_unet1 = {}
    for key, value in state_dict_site.items():
        if key == 'final.2.weight' or key == 'final.2.bias':  # 最后一个卷积层微调
            pass
        else:
            state_dict_site_new_unet1['unet_1.' + key] = value
    ...
    # setup optimizer
    params_G = filter(lambda p: p.requires_grad, G.parameters())
    ...
    optimizer_G = optim.Adam(params_G, lr=lr_G, betas=(beta1, 0.999))
